    ...
    return (
        <div className="card p-6 bg-light shadow-md rounded-lg">
            ...
            <div className="flex flex-wrap gap-2 mt-4">
                {[
                    { symbol: 'pi', label: '\\pi' },
                    { symbol: 'sqrt()', label: '\\sqrt{\\cdot}' },
                    { symbol: 'exp()', label: 'e^{\\cdot}' },
                    { symbol: '/', label: '\\frac{\\cdot}{\\cdot}', special: true },
                    { symbol: '**', label: 'x^{2}', special: true }
                ].map((btn, index) => (
                    <button
                        key={index}
                        onClick={() => insertSymbol(btn.symbol)}
                        className={`btn bg-primary hover:bg-primarylight text-dark px-3 py-1 rounded-md h-10 ${btn.special ? 'flex items-center justify-center' : ''}`}
                    >
                        <MathJax inline>
                            {btn.label}
                        </MathJax>
            ...
            <div className="flex flex-col pt-4">
                <button
                    className="btn bg-primary hover:bg-primarylight py-2 rounded-md text-dark"
            ...
            <div className="flex flex-col pt-4">
                <h3 className="text-primary font-bold text-lg">This is Displayed as:</h3>
